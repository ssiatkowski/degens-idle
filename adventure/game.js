(function() {
   ...
   ****************************************/
  const knowledgeSkills = ["tinkering", "intellect", "hacking", "cybernetics", "quantum"];
  ...
  function getInitialGameState() {
    return {
      ...
      skills: {
        endurance:   { level: 1, xp: 0, visible: true,  energyDrain: 2 },
        tinkering:   { level: 1, xp: 0, visible: true,  energyDrain: 1.5 },
        charisma:    { level: 1, xp: 0, visible: true,  energyDrain: 1.1 },
        alchemy:     { level: 1, xp: 0, visible: true,  energyDrain: 5 },
        travel:      { level: 1, xp: 0, visible: true,  energyDrain: 1 },

        intellect:   { level: 1, xp: 0, visible: false, energyDrain: 1.8 },
        perception:  { level: 1, xp: 0, visible: false, energyDrain: 1.5 },
        mechanics:   { level: 1, xp: 0, visible: false, energyDrain: 3 },
        combat:      { level: 1, xp: 0, visible: false, energyDrain: 8 },
        hacking:     { level: 1, xp: 0, visible: false, energyDrain: 5 },
        aiMastery:   { level: 1, xp: 0, visible: false, energyDrain: 4 },
        negotiation: { level: 1, xp: 0, visible: false, energyDrain: 8 },
        cybernetics: { level: 1, xp: 0, visible: false, energyDrain: 9 },
        quantum:     { level: 1, xp: 0, visible: false, energyDrain: 50 },
        omniscience: { level: 1, xp: 0, visible: false, energyDrain: 100 },
      },
      perks: {}
    };
  }
  ...
  function renderResources() {
    ...
    Object.keys(gameState.resources).forEach(rName => {
      ...
      div.setAttribute("data-tooltip", "Click to use +3 Energy");
  ...
  function updateSkillDisplay() {
    ...
    skillEls.forEach(el => {
      ...
      if (!sData) return;
      const xpTot = Math.pow(1.0125, sData.level - 1);
      ...
      let drainMsg = "";
      if (sName !== "travel") {
        drainMsg = `<br>Using this skill drains energy ${sData.energyDrain}x faster.`;
      }
      const tip = `${capitalize(sName)}: x${baseMult.toFixed(2)} speed${drainMsg}`;
    ...
    if (Array.isArray(task.skills)) {
      task.skills.forEach(sName => {
        if (gameState.skills[sName]) {
          baseDrain *= gameState.skills[sName].energyDrain;
        }
      });
    }
    return baseDrain;
  ...
  function gameLoop() {
    ...
    for (let i = 0; i < currentTasks.length; i++) {
      ...
      const fraction = delta / tData.totalDuration;

      // drain
      let zoneDrain = Math.pow(1.5, zone.id - 1);
      ...
      // XP
      const baseXP = tData.task.baseTime * xpScale;
      const xpThisTick = baseXP * fraction;

      const usedSkills = tData.task.skills || [];
      let xpEach = xpThisTick / (usedSkills.length || 1);
      ...
      if (tData.progress >= tData.totalDuration) {
        ...
        // knowledge => if knowledgeUnlocked & final rep & uses knowledge skill
  ...
  function showKnowledgeIfUnlocked() {
    ...
    if (gameState.knowledgeUnlocked) {
      ...
      // Add tooltip describing knowledge effect
      const tooltipStr = "Knowledge increases XP gain by<b>" + gameState.knowledge*0.1 + "%</b> for these skills:<br> -" 
  ...
  function showCopiumModal() {
    ...
    p.innerHTML = "Copium unlocked!<br>Tasks using skills:<br>-" + copiumSkills.join("<br>-") +
      "<br>now yield Copium.<br>If it reaches >9000, you reset with all resources and half knowledge lost.";    
    content.appendChild(p);
  ...
  function showCopiumBar() {
      // Optionally also un-hide the #copiumBarContainer
  ...
  function createSettingsButton() {
    const sBtn = document.getElementById("settingsButton");
    if (!sBtn) return;
  ...
  document.getElementById("restartButtonEnergy").addEventListener("click", () => {
    // user pressed "Restart" after energy is 0
