...
async function buyUpgrade(encodedUpgradeName, callUpdatesAfterBuying = true) {
    ...
    if (isAffordable(cost)) {
        ...
        if (name === "Complex Skill Trees") {
            showMessageModal('Sadly', "This marks the end of v0.86, but your journey is just getting started! The Hall of Power is only the beginning, with new challenges and excitement in every update. How are you enjoying the battles? Are they keeping you on your toes? Your feedback is key to shaping the game.<br><br>While you wait for the next update (just days away!), why not restart, try new tactics, and uncover hidden secrets? Share your thoughts on the battles and Hall of Power on Discord or through the feedback form. Let’s keep this adventure going!");
        }
